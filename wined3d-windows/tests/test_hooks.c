...
static void test_override_cooplevel_no_trigger_no_counter1(void)
{
    reset_state();
    override_counter = 3;
    ...
    ASSERT(override_counter == 3,
           "override_cooplevel: counter unchanged (no decrement)");
    ASSERT(level == 0x08,
           "override_cooplevel: level unchanged when trigger is sentinel and counter != 1");
}

static void test_override_cooplevel_counter1(void)
{
    reset_state();
    override_counter = 1;
    override_trigger = (DWORD)-1;
    ...
    ASSERT(override_counter == 1,
           "override_cooplevel: counter unchanged");
    ASSERT(level == (0x08 ^ 0x10),
           "override_cooplevel: 0x10 XORed when counter == 1");
}
...
static void test_override_cooplevel_with_trigger(void)
{
    reset_state();
    override_counter = 3;
    override_trigger = 5;   /* not sentinel → bit 0 toggled */
    ...
    ASSERT(override_counter == 3,
           "override_cooplevel: counter unchanged");
    ASSERT(level == (0x08 ^ 0x01),
           "override_cooplevel: 0x01 XORed when trigger is not sentinel");
}
...
{
    reset_state();
    /* Dummy resource. Donor-verified (Wine 6.0.4): bind_flags +0x30,
       bit WINED3D_BIND_RENDER_TARGET (0x20). */
    ...
    DWORD *flags = (DWORD *)(resource + 0x30);
    ASSERT((*flags & 0x20) != 0,
           "override_rtv: bit 0x20 set in resource->bind_flags");
}
...
int main(void)
{
    ...
    test_override_cooplevel_no_trigger_no_counter1();
    test_override_cooplevel_counter1();
