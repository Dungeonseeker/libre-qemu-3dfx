...
static void write_debug_msg(const char *msg);
...
static void debug_init(void)
{
    ...
    {
        parse_options( "fixme-all,err-all" );
    }

    write_debug_msg("--- wined3d debug log initialized ---\r\n");
    if (GetModuleFileNameA(NULL, modpath, sizeof(modpath)))
    {
        sprintf(init_msg, "Process: %s (PID: %lu)\r\n", modpath, GetCurrentProcessId());
        write_debug_msg(init_msg);
    }
}
...
static void write_debug_msg(const char *msg)
{
    ...
    /* Convert bare \n to \r\n for Windows XP Notepad */
    while (msg[i] && j < (int)sizeof(crlf_buf) - 3)
    {
        if (msg[i] == '\n' && (i == 0 || msg[i - 1] != '\r'))
        {
            crlf_buf[j++] = '\r';
    ...
    if (h != INVALID_HANDLE_VALUE)
    {
        WriteFile(h, crlf_buf, j, &written, NULL);
        CloseHandle(h);
    }
}
                             ...
                             const char *func, const char *format, va_list args )
{
    ...
    {
        hlen = sprintf( buf, "%s:%s:%s ", debug_classes[cls], channel ? channel->name : "?", func ? func : "?" );
        if (hlen < 0) hlen = 0;
        else if (hlen >= (int)sizeof(buf) - 1) hlen = sizeof(buf) - 1;
    }
    if (format && hlen < (int)sizeof(buf) - 1)
    {
        mlen = vsnprintf( buf + hlen, sizeof(buf) - 1 - hlen, format, args );
        if (mlen < 0) mlen = 0;
    }
    buf[sizeof(buf) - 1] = 0;
    write_debug_msg(buf);
